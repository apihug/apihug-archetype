...
public interface OrganizationEntityDSL extends DSL<OrganizationEntity> {
  Table table = Table.create("SYS_ORGANIZATION").as("_sys_organization");
  ...
  @Override
  default Object[] parameters(OrganizationEntity entity, Object[] carrier, int beginIndex) {
    carrier[beginIndex + 0] = entity.getParentId();
    carrier[beginIndex + 1] = entity.getOrganizationCode();
    carrier[beginIndex + 2] = entity.getOrganizationName();
  ...
  interface ColumnsMap {
    Map<String, ColumnMix> field2Column = 
     Map.ofEntries(
    	...
    	Map.entry("ORGANIZATIONCODE", Domain.OrganizationCode),
    	Map.entry("ORGANIZATIONNAME", Domain.OrganizationName),
  ...
  interface Domain {
    ColumnMix ParentId = ColumnMix.of(table, 
    	new Column().setFieldName("parentId")
    		...
    		.setClz("java.lang.Long")
    		.setType(Types.BIGINT)
    		.setDescription("父组织 ID")
    		.setUpdatable(true)
    		.setInsertable(true)
    		.setLength(255)
    		.setDefaultValue("PARENT_ID"));

    ColumnMix OrganizationCode = ColumnMix.of(table, 
    	new Column().setFieldName("organizationCode")
    ...
    ColumnMix OrganizationName = ColumnMix.of(table, 
    	new Column().setFieldName("organizationName")
    ...
    ColumnMix Status = ColumnMix.of(table, 
    	new Column().setFieldName("status")
    		.setName("STATUS_CODE")
    		.setClz("com.apihug.rad.infra.organization.OrganizationStatusEnum")
    		.setType(Types.VARCHAR)
    		.setDescription("组织状态")
    		.setUpdatable(true)
    		.setInsertable(true)
    ...
    List<ColumnMix> ALL = List.of(ParentId, OrganizationCode, OrganizationName, SortOrder, Status);
  ...
  interface Template {
    ParameterizedPreparedStatementSetter<OrganizationEntity> SETTER = 
    (ps, entity) -> {
    ps.setLong(1, entity.getParentId());
    ps.setString(2, entity.getOrganizationCode());
    ps.setString(3, entity.getOrganizationName());
    ps.setInt(4, entity.getSortOrder());
    ps.setString(5, entity.getStatus().name());
    };

    RowMapper<OrganizationEntity> MAPPER = new RowMapper() {
      @Override
      public OrganizationEntity mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        entity.setOrganizationCode(rs.getString(3));
        entity.setOrganizationName(rs.getString(4));
        entity.setSortOrder(rs.getInt(5));
        entity.setStatus(OrganizationStatusEnum.NA.mapFromName(rs.getString(6)));
