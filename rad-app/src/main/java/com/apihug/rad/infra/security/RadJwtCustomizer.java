...
/**
 *
 * JwtCustomizer responsible for user instance creation,
 * anonymous identity management, and JWT Token serialization.
 *
 * <p>As a core extension point of the ApiHug security framework, this class defines
 * the complete lifecycle management of {@link SchoolCustomer}, including identity creation
 * and custom claim handling during token encoding/decoding.</p>
 *
...
public class RadJwtCustomizer extends DefaultJwtCustomizer<RadCustomer> {
  public RadJwtCustomizer(JwtDecoder jwtDecoder, JwtEncoder jwtEncoder,
  ...
  public void prepareJwt(JwtClaimsSet.Builder claim, RadCustomer customer) {
    // Example: claims.claim(ROLES, customer.getRoles());;
   ...
   * <p>
   * This method is invoked following the decoding and validation of a JWT, providing an opportunity
   * to extract custom claims from the token and populate the corresponding fields of the customer
   * entity. Typical use cases include setting roles, privileges, tenant identifiers, or any other
   * custom attributes that were embedded in the JWT during token creation.
   * <p>
   * Example usage:
   * <pre>{@code
   * Map<String, Object> claims = jwt.getClaims();
   ...
   * }</pre>
   * <p>
   * Implementations <strong>must</strong> override this method to provide the actual enrichment logic.
   * The default implementation throws {@link UnsupportedOperationException} to indicate that it
   * is intended to be implemented by runtime contexts.
   *
  ...
  public RadCustomer postDecode(RadCustomer customer, Jwt jwt) {
    // Populate custom fields here, then return the customer;
    return customer;
  }
   ...
   * <p>
   * This method is intended to be used after extracting the tenant ID from a JWT claim,
   * where the raw value may be of various numeric types (e.g., {@link Long}, {@link Integer})
   * or a  {@link String}. The implementation is responsible for converting the provided {@code tenantId}
   * to the appropriate type expected by the customer entity, thereby avoiding runtime
   * {@link ClassCastException} or other type mismatch issues.
   * <p>
   * The method is declared abstract to enforce that concrete subclasses provide a
   * type‑safe assignment strategy tailored to the specific {@code T} customer type.
   *
   * @param customer the customer instance (partially initialized) that will receive the tenant ID
   * @param tenantId the tenant identifier extracted from the JWT; may be a {@link Number},
   ...
   * <p>
   * This method is intended to be used after extracting the customer ID from a JWT claim,
   * where the raw value may be of various numeric types (e.g., {@link Long}, {@link Integer})
   * or a string. The implementation is responsible for converting the provided {@code customerId}
   * to the appropriate type expected by the customer entity, thereby avoiding runtime
   * {@link ClassCastException} or other type mismatch issues.
   * <p>
   * The method is declared abstract to enforce that concrete subclasses provide a
   ...
   * <p>
   * This method determines the source from which the JWT should be retrieved. Common sources include
   * the HTTP {@code Authorization} header (default), a request parameter, or a cookie. The
   * implementation is designed to work across different web stacks: the {@code context} parameter
   * can be either a {@link org.springframework.http.server.reactive.ServerHttpRequest} (for reactive
   * applications) or a {@link jakarta.servlet.http.HttpServletRequest} (for servlet‑based
   * applications).
   * <p>
   * Subclasses may override this method to support alternative token locations, such as extracting
   * the token from a query parameter or a cookie instead of the default header.
   *
   * @param http    the {@link HttpProtocolAdapter} providing protocol‑specific access methods
   * @param key     the key used to locate the token (e.g., the header name, parameter name, or
   *             cookie name)
   ...
   * @return the raw JWT string, or {@code null} if the token is not present in the expected location
   */
  @Override
